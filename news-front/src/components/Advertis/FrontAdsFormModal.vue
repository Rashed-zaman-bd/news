...
const form = ref({
    ...
    provider: '',
    link_url: '',
    placement: 'top' as 'top' | 'middle' | 'middle-two' | 'middle-three' | 'middle-four' | 'middle-five' | 'middle-six' | 'middle-seven' | 'middle-eight' | 'middle-nine' | 'middle-ten' | 'sidebar' | 'sidebar-two' | 'sidebar-three' | 'sidebar-four' | 'sidebar-five' | 'sidebar-six',
    sort_order: 0,
    is_active: true,
    starts_at: '',
    ends_at: '',
...
const submit = async () => {
    ...
    try {
        if (isEdit.value && props.advertisement) {
            ...
            await api.post(`/front-ads/${props.advertisement.id}`, formData)
        } else {
            await api.post('/front-ads', formData)
        }
