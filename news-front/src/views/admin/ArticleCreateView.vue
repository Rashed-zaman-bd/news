<template>
    <div class="p-4 md:p-6">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-6">
            <h1 class="text-2xl font-bold text-gray-800">পর্যালোচনাধীন আর্টিকেল</h1>
            <button
        ...
        <div class="bg-white rounded-lg shadow p-4 mb-4">
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
                ...
                <select
                    ...
                    @change="fetchArticles(1)"
                    :disabled="!filters.category_id || !activeSubCategories.length"
...
const DEFAULT_STATUS = 'draft,pending';
...
const filters = reactive({
    status: DEFAULT_STATUS, // always restricted to draft/pending on this page
    search: '',
    category_id: '',
    sub_category_id: '',
    author_id: '',
...
const fetchArticles = async (page = 1) => {
    ...
    try {
        const { data } = await api.get('/admin/articles', {
            params: {
                page,
                status: filters.status || DEFAULT_STATUS, // never send empty — always draft/pending scoped
